...
### 2.4 Настрой таблицы через Apps Script

1. В таблице: **Расширения → Apps Script**
2. Скопируй содержимое `scripts/Code.gs` и вставь в редактор
3. Сохрани проект (Ctrl+S / ⌘S)
4. Запусти функцию `setupSheetsSafe()` из выпадающего списка
5. Дай разрешения при запросе (скрипту нужен доступ к таблице)

**После запуска в меню появится «🧮 Kesha» со всеми командами.**

**Создаются/проверяются листы:**
| Лист | Колонки | Назначение |
|------|---------|------------|
| Transactions | A-L (12) | Все операции с поддержкой переводов |
| Budgets | A-D | Бюджеты и лимиты по категориям |
| Categories | A-C (Type + Name + Color) | Кастомные категории и их цвета |
| Settings | A-B (Key + Value) | Настройки пользователя |
| Rules | A-D | Правила авто-категоризации |
| Accounts | A-I (9) | Реестр счетов с балансами |
| Recurring | A-X (24) | Регулярные платежи и подписки |

> ⚠️ **Важно:** `setupSheetsSafe()` НЕ удаляет существующие данные.
> Он только создаёт недостающие листы и добавляет недостающие заголовки.
>
> **Никогда не запускай `resetSheetsDangerously()` на рабочей таблице** —
> эта функция полностью очищает все листы и нужна только для новой пустой таблицы.

### Миграция: BankAccounts → Accounts

Все счета теперь в листе Accounts (реестр: ID, тип, валюта, баланс).
Старый лист BankAccounts больше не создается и не используется.
После проверки /banks удали его вручную: правый клик по вкладке → Удалить.
